...
public class RoadStatusController implements SEIPicture {
    ...
    private static List<Picture> store = new CopyOnWriteArrayList<Picture>() {
        {
            add(new Picture(1));
            add(new Picture(2, "qwer.jpg", "cats fail", new Point(30, 50)));
            add(new Picture(3, "aaa.jpg", "tortle fail", new Point(30, 50),
            ...
            add(new Picture(4, "ssss.jpg", "dogs fail", new Point(30, 50)));
            add(new Picture(5, "rad.jpg", "rad fail", new Point(42, 34),
    ...
    public ResponseEntity<Long> methodPutPicture(@RequestParam Picture picture, final HttpServletResponse response) {
        ...
        /*
         * for (int index = 0; index < store.size(); index++) { if (store.get(index).getId() == picture.getId()) { store.set(index, picture); return (ResponseEntity<Void>) ResponseEntity.ok(); } } return (ResponseEntity<Void>)
